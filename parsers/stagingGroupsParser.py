...
for index in range(1, sh.nrows):
    if isinstance(sh.cell_value(index,2), str):
        print("\"" + sh.cell_value(index,1) + "\" : { \"survival_rate\": \"" + (sh.cell_value(index,2)) + "%\", \"action_group\": \"" + str(int(sh.cell_value(index,0))) + "\", \"pathologic_stage\": \"" + str(sh.cell_value(index,3)) + "\", \"clincal_stage\": \"" + str(sh.cell_value(index,4)) + "\"}, ")
    else:
